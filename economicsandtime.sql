...
WITH IMD_Risk_Profiles AS (
    SELECT 1 AS imd_decile,
    1.35 AS ckd_prevalence_multiplier, 
    1.40 AS mh_overlap_multiplier UNION ALL
    SELECT 2,             1.25,                           1.30                           UNION ALL
    SELECT 5,             1.00,                           1.00                           UNION ALL
...
Calculated_Intervention AS (
    ...
    JOIN IMD_Risk_Profiles r 
    ON p.imd_decile = r.imd_decile
    CROSS JOIN (SELECT 12000.00 AS base_cost) 
    -- Example standardized baseline cost placeholder
